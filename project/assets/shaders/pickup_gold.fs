...
uniform float pickupFlash;
uniform float pickupSparkle;
    ...
    vec3 albedoLinear)
{
    ...
    float specularTerm = pow(max(dot(n, halfDir), 0.0), 64.0);

    float diffuseLighting = diffuse * (1.0 - 0.45 * roughness);
    vec3 diffuseColor = albedoLinear * diffuseLighting * mix(1.0, 0.5, metallic);
    vec3 specColor = vec3(specular * specularTerm * (1.0 - 0.35 * roughness));
...
void main()
{
    ...
    float flash = max(pickupFlash, 0.0);
    float sparkle = max(pickupSparkle, 0.0);
    float fresnel = pow(1.0 - max(dot(n, viewDir), 0.0), 3.0);
    vec3 goldHighlight = vec3(1.0, 0.93, 0.52);

    litColor *= flash;
    litColor += goldHighlight * (fresnel * (0.25 + sparkle));
    litColor += goldHighlight * sparkle * 0.05;
